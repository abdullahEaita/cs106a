...
def get_pixel_dist(pixel, red, green, blue):
    image = SimpleImage.blank(20,20,'green')
    for y in range(image.height):
        for x in range(image.width):
            avverage = (pixel.red + pixel.blue + pixel.green) / 3
            pixel = image.get_pixel(x, y)
            return avverage
    # doctest = math.sqrt(red_diffre**2 + green_differ**2 + blue_diffre**2)
    # math.sqrt(red_difference ** 2 + green_difference ** 2 + blue_difference ** 2)

            # color_diff = math.sqrt((pixel.red - red)**2 + (pixel.green - green)**2 + (pixel.blue - blue)**2)
            # return color_diff
